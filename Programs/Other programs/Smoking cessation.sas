...
if smoke1yr = 3 then smoke_2cat=l;
...
where smoke_2cat ne .;
table ehediab ehestrok ehearth eeatmmea edrkmcof ;run;
...
proc freq data=check1; table smoke_2cat*lit_3cat*asex;run;

proc glimmix data=check1;
/*women*/
where asex=1;
 class pct33wealthc educ incCtry lit_3cat ;
 model smoke_2cat (ref="2") = lit_3cat  
/ dist=binary link=logit ddfm=bw solution oddsratio  or(label);
 random commid / solution;
 estimate 'Tertitle 1 vs tertile 3' lit_3cat 1 0 -1 / or;
run;
...
if Subgroup eq " " and row not in (1,6,13,19) then do;
	HR1=HR;
	cil1=cil;
	ciu1=ciu;
end;

if Subgroup  ne " " and row not in (1,6,13,19) then do;
	HR2=HR;
	cil2=cil;
	ciu2=ciu;
end;
...
layout lattice /
columns=3 columnweights=(0.25 0.15 .30);

/*--Column headers--*/
sidebar / align=top;
layout lattice /
rows=2 columns=3 columnweights=(0.01 0.20 0.35 )
...
entry halign=left textattrs=(color=black size=7pt) "<--Low --" halign=right textattrs=(color=black size=7pt) "--High -->" /location=outside valign=bottom;
referenceline y=ref / lineattrs=(thickness=_thk color=_color);
...
referenceline x=1/ lineattrs=(pattern=dot thickness=.05 color=black);
legend;
endlayout;
...
ods graphics / reset width=8.5in height=5.4in imagename='smokegraph' border;
proc sgrender data=Forest2 template=Forest sganno=anno ;
format HR misblank7.4  p pformat.; 
dynamic _color='Azure' _headercolor="lightblue" _thk=14 ;
title;
run;
ods rtf close;
