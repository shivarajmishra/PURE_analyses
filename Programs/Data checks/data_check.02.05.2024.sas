...
/*Data checks*/
options nofmterr;
libname check1 '\WARC\PURE Health Literacy\Analysis\Data';
...
proc freq data=check1;table ehehdis*ctry;format country countryf.;run;

data check1_epoch;
set check3.complete_epoch2;run;
proc freq data=check1_epoch;table ehehdis*country;format country countryf.;run;
data check_a;set check1_epoch;if country=30;run;
proc freq data=check_a;table eheclund ehehdis ehediab ehestrok ehearth ehelucan ehemocan ehehdnsm eheprebi ehelbirw 
edomexe eeatmfru eeatmgve eeatmdpr eeatmfis  eredfatm eredsalt;run;

/**for country=30, all the variables are missing, so country=30 is dropped from analysis, this would 
effectively limit the sample size to 13912**/
/**there is a tiny bit of difference in total head counts for countries: Pakistan (102 in old dataset vs 230 in new), and Philippines (87 vs 150).*/
title "New dataset";
proc freq data=check1_epoch;where country NE 30; table eheclund ehehdis*bpmedm  ehediab ehestrok ehearth ehelucan ehemocan ehehdnsm eheprebi ehelbirw 
edomexe eeatmfru eeatmgve eeatmdpr eeatmfis  eredfatm eredsalt;run;

title "Old dataset"; /*use the dataset test2, see the steps below for caculation*/
proc freq data=test2;where country NE 30; table eheclund ehehdis*bpmedm ehediab ehestrok ehearth ehelucan ehemocan ehehdnsm eheprebi ehelbirw 
edomexe eeatmfru eeatmgve eeatmdpr eeatmfis  eredfatm eredsalt;run;

data check2;
set check1.base_adult; 
run;
